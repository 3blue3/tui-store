...
bool choice_prompt(char *prompt){
 ...
 bool prompt_shelf(char *buffer, int buffer_size ){
   ...
   if (buffer_size < 3) perror("to small buffer for shelf");
   if (prompt_string(buffer, 3, "Shelf: ", 
		     "Enter a valid shelf (format <char><num><num>)",
		     "Invalid shelf, try again?")){
     return true;
   } else return false;
 }
...
bool prompt_string(char *buffer, int buffer_size,
		   ...
		   char *prompt_again){ 
  // Prompt for input of a string, reading input until
  // the string is a string (as per is_string) and the
  // check function is satisfied.
  ...
  do {
    ...
    printf("%s", buffer);
    puts("");
  }while (!not_empty(buffer) &&
	  choice_prompt(prompt_again));
